...
# ===== ПОЛНЫЙ СКРИПТ РАЗВЕРТЫВАНИЯ НА НОВЫЙ СЕРВЕР =====
# ⚠️ ВНИМАНИЕ: Этот файл содержит пароли в открытом виде!
# Используйте переменные окружения или deploy-full.sh.safe!
# 
...
SERVER_IP="${DEPLOY_SERVER_IP:-185.221.213.129}"
SERVER_USER="${DEPLOY_SERVER_USER:-root}"
SERVER_PASS="${DEPLOY_SERVER_PASS:-ToneXXl999!!!}"
...
apt-get install -y fail2ban ufw -qq
...
echo "📦 Шаг 6.1: Восстановление базы данных из дампа..."
ssh_cmd "bash -s" << 'DB_RESTORE'
set -e
export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin

# Установка PostgreSQL 16 client для восстановления дампа
apt-get install -y postgresql-client-16 -qq 2>/dev/null || apt-get install -y postgresql-client -qq
...
if [ -f /root/casino_backup_from_server_20251216_174734.dump ]; then
    ...
    chmod 644 /root/casino_backup_from_server_20251216_174734.dump
    # Пробуем через pg_restore 16
    if command -v pg_restore &> /dev/null; then
        sudo -u postgres pg_restore --no-owner --no-privileges -d casino /root/casino_backup_from_server_20251216_174734.dump 2>&1 | tail -20 || {
